...
ggplot(data.freqs.ap.threads,aes(x=Threads,y=Evaluations/Time,group=Threads))+geom_boxplot(fill='yellow',notch=TRUE)+theme_tufte()+scale_x_continuous(name="Threads",breaks=c(2,4,6,8))+labs(x="Generation.Gap",y="Evaluations/s",title="Scaling")+theme(text = element_text(size=15))
ggsave("evaluations-second-freqs-ap.png", height=6, units="cm" )
