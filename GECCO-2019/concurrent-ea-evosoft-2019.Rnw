...
processors and cores on a given machine. The paralellization of nature-based 
...
Using these native capabalities give developers the advantage of
having less error-prone implementations which are more readable and consice
