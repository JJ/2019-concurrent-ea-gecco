...
processors on a given machine. The paralellization of nature-based 
optimization algorithms has been an active field, allowing researchers
to solve complex optimization problems having a high computanional 
...
parallization by using libraries such as OpenMP \cite{dagum1998openmp}
...
constructs to programing languages with the intention of simplifying 
the development of multi-threaded programs. Languages such as Go 
...
to laverage multi-threding capabilities for IO intensive tasks.
The theoretical support used by these implementations is based 
on Hoare's {\em Communicating Sequential Processes} (CSP) \cite{Hoare:1978:CSP:359576.359585}.
In CSP, concurrent programs communicate on 
...
The next figures chart the results obtained for the Royal Road
function. We run the experiment as above, but in this case there were
