...
\author{Anonymous Author 1\inst{1} \and
Anonymous Author 2\inst{1} \and
Anonymous Author 3\inst{1} \and
Anonymous Author 4\inst{1} }
%
\authorrunning{Author 1, Author 2, Author 3, Author 4}

\institute{University 1, city1, country1 
\email{aa@email.org}\\
}
...
where many processes and threads can coexists at the same time, which
...
The best way to do so, and to think of processing at a high level, is
to match those capabilities at an abstract level by languages
that build on them so that high-level algorithms can employ them without
...
These languages, that include Perl 6, Go, Scala and Julia are characterized by the presence of
...
by a section on the design of concurrent EAs in Perl 6
\ref{sec:design}. Experimental results are presented next in Section
\ref{sec:exp}; finally, we will present our conclusions in Section
\ref{sec:conclusions}. 
