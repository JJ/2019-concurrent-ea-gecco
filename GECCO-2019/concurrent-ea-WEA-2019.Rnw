...
groups of threads and two channels. The two groups of threads perform the following
...
The pattern these charts exhibit is very similar to the one for
...
Even so, results were quite different. They
...
message size and actual number of threads must be considered.
...
On the other hand, the messaging strategies proposed 
here can be used only if the problem
can be represented via a binary data structure. New mechanisms will
have to be devised for floating-point representation, or more
...
case. EDAs can also be extended to
other types of data, so this is is something that can be also
considered. %in the case of lossy messages such as this one, they could
