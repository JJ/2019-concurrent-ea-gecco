...
%... part of them. Therefore, when designing a concurrent algorithm, 
% some attention must be devoted to the layout of the communication
% architecture in order to gain performance and achieve an efficient 
% design. - Juanlu
Concurrent, stateless evolutionary algorithms
% I will supress "stateless" here. It's gonna be distracting at
% this point to the audience. So just leave "Concurrent evolutionary algorithms..." - Juanlu
...
its capabilities. 
% ... itself. The evolutionary algorithm is implemented using a concurrent 
% programming language in order to fully exploit the algorithm's capabilities. - Juanlu
Eventually, we will try to prove 
% try to prove -> show. (either we prove or we don't, so I think show is more accurate) - Juanlu
...
increasing the number of threads in order to find out the limits of
% the limits of -> upper bounds for - Juanlu
...
% What about this one: 
% Despite the emphasis on laveraging newer hardware features
% with best-suited software techniques,... - Juanlu
...
computers. 
Concurrent programming has a physical basis: the existence
...
within those same processors.
% ... to many computers. However, concurrent programming seems to be the best option
% if we are dealing with a multi-core processor architecture where many processes and threads can coexists at the same time. - Juanlu
